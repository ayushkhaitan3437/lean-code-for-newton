...
/-- The finite support points `(i, ord (coeff i))` of a polynomial.  Only
coefficients with a finite `ord`-value are recorded; when `ord` is a genuine
additive valuation this omits exactly the zero coefficients, which is the
paper's convention. -/
...
/-- `f` has the Newton polygon described by `data`: strictly increasing
maximal-segment slopes, total horizontal length equal to `natDegree f`, and
the coefficient-support epigraph equal to the polygon generated by the
segment vertices.  Since the vertex chain always starts at `x = 0`, this
predicate implicitly requires `f.coeff 0 ≠ 0` (the paper's normalization for
Newton polygons); a polynomial divisible by `X` never satisfies it. -/
